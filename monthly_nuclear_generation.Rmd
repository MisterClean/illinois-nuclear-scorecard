...
long_dataframe <- final_dataframe %>%
...
long_dataframe$value <- long_dataframe$value / 1000

# Create the line chart with values in GWh
ggplot(long_dataframe, aes(x = date_eastern, y = value, group = variable)) +
...
```
```{r}
# Create a kable table and style it
table_output <- kable(load_and_nuclear_report, "html", align = c('l', 'c', 'c'),
                      col.names = c("Date", "Total Electricity Consumed (MWh)", "Estimated Total Nuclear Electricity Generated (MWh)")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = F, 
                position = "left") %>%
  column_spec(1, width = "150px") %>% # Adjust column 1 width
  column_spec(2, width = "200px") %>% # Adjust column 2 width
  column_spec(3, width = "200px") %>% # Adjust column 3 width
